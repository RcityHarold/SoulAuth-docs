...
在要紧处两者都跨副本共享。锁定状态在数据库里。限流跑两层：先过进程内那关，
再过 SurrealDB 支撑的共享计数 —— 后者覆盖所有带显式规则的端点：登录、注册、
密码重置、邮箱验证、MFA 挑战。走一般默认规则的端点留在进程内，
用普通流量上的 N 倍上限换「不给每个请求加一次数据库往返」。

共享后端出错时**放行**并留一条 error 日志，进程内那层仍在生效。
反过来做（数据库一抖就拒登录）等于给自己造一个拒绝服务开关 ——
这是一次明确的取舍，在调用点有注释。
