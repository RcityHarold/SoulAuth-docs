...
一个 Rust 二进制，一个数据库，以及一组**被强制执行**而非仅作建议的边界。
...
一套架构中真正有信息量的往往是那些限制，因此逐层列出如下。

### 协议边缘层

OIDC 端点、客户端 API、管理 API 的 HTTP handler。它把 wire 格式转成领域调用再转回去。

这一层不做认证判定。某个 handler 如果自行判断「这看上去没问题」，就等于把一条
认证规则放在了没有任何测试会去检查的位置。
...
否则每一条读取路径都会成为潜在的认证绕过。

### 认证核心层

口令校验、TOTP、AI 主体的挑战—应答、联邦回调、账号锁定。

这一层不授予权限。它产出的只是一句关于身份的陈述，仅此而已。参见
...
以下各条都被强制执行，而不是理想目标。每一条都写出了守住它的那个测试。
...
最后一条没有对应测试，因为它是结构性的：SoulAuth 不发出任何 DDL，两个 SQL 文件
由部署者导入。一个认证服务如果持有改写自身表结构的权限，就越过了本项目划定的边界。
