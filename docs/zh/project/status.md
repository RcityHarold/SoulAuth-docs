...
| | 对你意味着什么 |
|---|---|
| <Status kind="planned" /> `ActorIdentity` 上的 RBAC | AI 主体能认证，但它的会话不带权限。只到得了 `/api/actors/me`，别处都不行。 |
| <Status kind="planned" /> 收口的 `Credential` 对象 | 口令与 TOTP 仍在遗留 `user` 表上。你调用的东西不变，但模型没完全落地。 |
| <Status kind="planned" /> 防篡改审计 | 审计日志是一张普通表。**不要拿它当证据。** |
| <Status kind="planned" /> 人类事件归因到身份根 | 人类审计行以 `user_id` 为键。AI 主体的事件已经归因到身份根了。 |
| <Status kind="planned" /> 物化的 `AuthenticationResult` | 只是内部 runtime fact。到你手上的（会话令牌、OIDC claims）是稳定的。 |
...
生命周期内稳定——不是 OIDC Core 期待的「永不重新分配」。作为具名 caveat 记在
...
看到一个没写名字的，那是缺陷——`scripts/check-status.mjs` 本该拦住它。
