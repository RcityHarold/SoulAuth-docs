...
它存在的目的只有一个：说清**哪个事实归谁所有**。共享事实源的系统会失去可分离性，
而一个能定义「某人是谁」的身份提供方，已经不再是身份提供方。
...
这条区分只在一个时刻显出重量：有人提议让 SoulAuth「顺便也存一点」Soulseed 的状态，
理由是反正它就在手边。两个系统就是这样变成一个有两个名字的系统的。

::: warning 它是受控声明
`canonical_actor_ref` **默认不暴露**给第三方 OIDC 客户端。一个指向另一套系统身份域的
指针属于集成状态，不是公开档案字段；默认发布它，等于把部署拓扑泄露给每一个依赖方。
:::
...
<Status kind="planned" /> 本页描述的是几个系统之间的关系，不说明当前 Release
实现了这层关系中的哪些部分。那件事见[项目状态](/zh/project/status)。
