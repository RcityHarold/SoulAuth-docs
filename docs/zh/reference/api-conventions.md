...
跨所有端点成立的部分。读完这一页，后面的参考就只是一张路径清单。

<ContractNote file="openapi.yaml" />

## 基地址

一切相对于 `APP_URL`——那是公开地址，同时也是 OIDC issuer。两者必须逐字符一致。

## 认证

三种机制。它们**不可互换**，混淆它们是最常见的接入错误：
...
- **会话令牌不是 OIDC 访问令牌。** 它们都是同一个头上的 bearer 令牌，
  这正是区别容易丢失的原因。送错那个会得到一个看起来像凭证问题的 401。
...
- 个别错误带已文档化的补充字段——`missing_permission` 带 `required_permission`，
  `account_locked` 带 `locked_until_seconds`。它们是附加的。

<Status kind="tested" guard="conformance::j6" /> 守住没有任何端点返回第二种形状
——包括它取代的那个真实缺口：曾经有一批端点用裸状态码作答、**响应体为空**，
调用方既拿不到码也拿不到说明。
...
`501` 是刻意的。一个没配 OAuth 凭证的部署上的联邦登录端点不是「未找到」——
路由在、语义明确，只是运维还没填配置。`404` 会让人去查错误的方向。

## 标识符

记录标识符形如 `table:key`，例如 `actor_identity:lnhl…`。路径里接受带前缀与
不带前缀两种写法——URL 里的冒号会被很多客户端库转义，而两种写法含义相同。
...
支持分页的列表端点收 `limit` 与 `offset`。响应是裸数组或裸对象——
没有信封，没有 `data` 包装。
